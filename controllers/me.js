module.exports = (container) => {
  ...
  const pool = container.get("db");
  const validators = container.get("validators");
  const uploadToImgBB = container.get("imageUploader");
  const { logError } = container.get("logger");
  ...
  const getUserAds = async (req, res) => {
    ...
    try {
      ...
      res
        .status(200)
        .json({ rowCount: response.rowCount, rows: response.rows });
    } catch (err) {
      logError(err);
      res.status(500).json({
        error: err.message,
    ...
    `;
      ...
      if (price) {
        if (!validators.ValidateNumber(price))
          return res
            .status(400)
            .json({ error: "please provide a valid price" });
      ...
      if (category_id) {
        ...
        if (Number(category_id) < 1 || Number(category_id) > 8)
          return res.status(400).json({
            error: "please provide a valid category id betwen 1 and 8",
          });
